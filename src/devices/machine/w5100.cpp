...
TIMER_CALLBACK_MEMBER(w5100_base_device::retry_timer)
{
	...
	if (cmd == TIMER_ARP)
	{
		...
		if (timeout)
		{
			...
			else
			{
				...
				{
					// UDP / IPRAW update tx_rd and fsr at this point.
	...
	if (cmd == TIMER_TCP_RESEND)
	{
		...
		// tcp timer backs off, eg .2 sec, .4, sec, .8 sec, 1.6 sec ... 
		auto pd = m_retry_timers[sn]->period() * 2;
		m_retry_timers[sn]->adjust(pd,  param, pd);
		...
		case 1:
			...
			switch(bank)
			{
				...
				case 7:
					...
					return m_socket_registers[bank & 0x03][offset];
					break;
				default:
					return m_common_registers[offset];
					break;
			}
			break;
		case 2:
	...
	switch(offset >> 13)
	{
		...
		case 1:
			...
			switch(bank)
			{
				...
				case 6:
				case 7:
					write_socket_register(bank & 0x3, offset, data);
					break;
				default:
					write_common_register(offset, data);
					break;
			}
			break;
		case 2:
...
void w5100_base_device::socket_send(int sn)
{
	...
	/*
	  TX_WR and FSR are updated immediately, before any ARP happens.
	  while sending (or after ARP timeout), FSR is adjusted.
	*/
...
void w5100_base_device::socket_listen(int sn)
{
	...
	m_sockets[sn].reset_tcp();
	m_sockets[sn].rcv_wnd = m_sockets[sn].rx_buffer_size;
...
bool w5100_base_device::socket_arp(int sn)
{
	...
	{
		// W5100S doesn't have SR_ARP status.
...
void w5100_base_device::handle_arp_reply(const uint8_t *buffer, int length)
{
	...
	if (m_device_type == dev_type::W5100S)
	{
		...
		if (m_sockets[kSLSocket].arp_active && m_sockets[kSLSocket].arp_ip_address == ip)
		{
			memcpy(m_common_registers + SLPHAR0, arp + o_ARP_SHA, 6);

			m_sockets[kSLSocket].arp_valid = true;
			m_sockets[kSLSocket].arp_active = false;
...
void w5100_base_device::handle_arp_request(const uint8_t *buffer, int length)
{
	...
	static const int MESSAGE_SIZE = 60;
...
void w5100_base_device::send_icmp_unreachable(uint8_t *buffer, int length)
{
	...
	int ihl = (buffer[14 + o_IP_IHL] & 0x0f) << 2;
	...
	message[25] = 0;
	memcpy(message + 26, m_common_registers + SIPR0, 4); // source ip
	memcpy(message + 30, buffer + 14 + o_IP_SRC_ADDRESS, 4); // destination ip
	...
	message[37] = 0;
	message[38] = 0; // unused (4 bytes)
	message[39] = 0;
	message[40] = 0;
	message[41] = 0;

	memcpy(message + 42, buffer + 14, 8 + ihl);
	...
	// icmp crc
	crc = util::internet_checksum_creator::simple(message + 34, 8 + ihl + 8);
	message[36] = crc >> 8;
	message[37] = crc;
...
void w5100_base_device::tcp_send(int sn, bool retransmit)
{
	...
	uint16_t write_ptr = retransmit ? snd_nxt : read16(socket + Sn_TX_WR0);
...
void w5100_base_device::tcp_segment(int sn, const uint8_t *buffer, int length)
{
	...
	LOGMASKED(LOG_TCP, "flags=%02x (%s), ack=%u seq=%u length=%u\n",
		flags, tcp_flags_to_string(flags), seg_ack, seg_seq, seg_len);
